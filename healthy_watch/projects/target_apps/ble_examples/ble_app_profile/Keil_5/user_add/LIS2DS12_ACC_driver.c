...
u8_t LIS2DS12_ACC_ReadReg(u8_t Reg, u8_t* Data) 
{
  ...
  //i.e.: *Data = SPI_Mems_Read_Reg( Reg );
	...
	*/
	i2c_error_code code = i2c_eeprom_read_byte(Reg, Data);
...
u8_t LIS2DS12_ACC_WriteReg(u8_t Reg, u8_t Data) 
{
  ...
  return MEMS_SUCCESS;                                         //[Example]
	*/
	i2c_error_code code = i2c_eeprom_write_byte(Reg, Data);
	if (code == I2C_NO_ERROR)
  {
		return MEMS_SUCCESS;  
	}else{
...
status_t  LIS2DS12_ACC_W_LIR(LIS2DS12_ACC_LIR_t newValue)
{
  ...
  value &= ~LIS2DS12_ACC_LIR_MASK;                      //latched interrupt
  value |= newValue;
