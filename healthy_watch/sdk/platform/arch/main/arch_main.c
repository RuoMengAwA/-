...
static void init_LIS2DS12_Pedometer(void)
{
  ...
  LIS2DS12_response = LIS2DS12_ACC_W_ODR(LIS2DS12_ACC_ODR_HR_14bit_100Hz);
  if(LIS2DS12_response==MEMS_ERROR) while(1); //manage here comunication error
...
u16_t CalorieNum=0 ;
unsigned char hr_capture=0;//capture的心率值
extern unsigned char dghData[20];
extern unsigned char	heart_beat_flag ;
extern unsigned char blood_oxygen_flag;
void DataConversion(void)   //数据转换 cust1_task.c
{
	CalorieNum  = Number_Of_Steps<<2;
	Steps[0] = (unsigned char)((Number_Of_Steps/10000));
	Steps[1] = (unsigned char)((Number_Of_Steps%10000/1000));
	Steps[2] = (unsigned char)((Number_Of_Steps%1000/100));
	Steps[3] = (unsigned char)((Number_Of_Steps%100/10));
	Steps[4] = (unsigned char)((Number_Of_Steps%10));
	Calorie[0] = 0;
	Calorie[1] = (unsigned char)((CalorieNum/100000));
	Calorie[2] = (unsigned char)((CalorieNum%100000/10000));
	Calorie[3] = (unsigned char)((CalorieNum%10000/1000));
	Calorie[4] = (unsigned char)((CalorieNum%1000/100));
//	Heart_Rate[0]=(unsigned char)((hr_capture%100/10))*2;
//	Heart_Rate[1]=(unsigned char)((hr_capture%10))*2;
//	cal_numb_dis[4] = 154;
//	cal_numb_dis[5] = (unsigned char)((cal_cnt%100/10)<<1);
//	cal_numb_dis[6] = (unsigned char)((cal_cnt%10)<<1);
}
...
int main_func(void)
{
    sleep_mode_t sleep_mode;
		...
		unsigned char  who_am_i = 0x0 , return_code = 0x00;
    //global initialise
    system_init();
		i2c_eeprom_init(0x1D, I2C_FAST, I2C_7BIT_ADDR, I2C_1BYTE_ADDR);
		/* Read WHO_AM_I  and check if device is really the correct one */
		LIS2DS12_ACC_R_WHO_AM_I_BIT(&who_am_i);
		if (who_am_i != LIS2DS12_ACC_WHO_AM_I)
			while(1); //manage here comunication error

		/* Software reset the LIS2DS12 device */
		return_code=LIS2DS12_ACC_W_SOFT_RESET(LIS2DS12_ACC_SOFT_RESET_ON);
		if(return_code!=MEMS_SUCCESS)
			while(1);
     ...
     ************************************************************************************
     */
//		uart2_write(&dgh,1,NULL);  //just for test by dgh
            ...
            GLOBAL_INT_START();
				...
				if(StepFlag)
				{
					...
					if(!OLED_dis_flag)
					{
						...
						dghData[1]=(u8_t )Number_Of_Steps;
//						spi_flash_block_erase(0x03fe00,SECTOR_ERASE);
//						dgh=spi_flash_write_data(dghData, 0x03fe00,10);
					}
				}
...
/**
 ****************************************************************************************
 * @brief Used for processing of asynchronous events at 搖ser� level. The
